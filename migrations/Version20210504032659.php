 ...
 */
final class Version20210504032659 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__app_users AS SELECT id, username, password, email, is_active, is_verified FROM app_users');
        $this->addSql('DROP TABLE app_users');
        $this->addSql('CREATE TABLE app_users (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(25) NOT NULL COLLATE BINARY, email VARCHAR(254) NOT NULL COLLATE BINARY, is_active BOOLEAN NOT NULL, password VARCHAR(64) NOT NULL, is_verified BOOLEAN NOT NULL, roles CLOB NOT NULL --(DC2Type:json_array)
        )');
        $this->addSql('INSERT INTO app_users (id, username, password, email, is_active, is_verified) SELECT id, username, password, email, is_active, is_verified FROM __temp__app_users');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__app_users AS SELECT id, username, password, email, is_active, is_verified FROM app_users');
        $this->addSql('DROP TABLE app_users');
        $this->addSql('CREATE TABLE app_users (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(25) NOT NULL, email VARCHAR(254) NOT NULL, is_active BOOLEAN NOT NULL, password VARCHAR(64) DEFAULT NULL COLLATE BINARY, is_verified BOOLEAN DEFAULT \'TRUE\' NOT NULL)');
        $this->addSql('INSERT INTO app_users (id, username, password, email, is_active, is_verified) SELECT id, username, password, email, is_active, is_verified FROM __temp__app_users');
        $this->addSql('DROP TABLE __temp__app_users');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_C2502824F85E0677 ON app_users (username)');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_C2502824E7927C74 ON app_users (email)');
